...
public class ShareService {
    ...
    public List<SharePostResponseDto> findFirstCategoryPosts() {
        ...
        List<SharePost> entities = sharePostRepository.findByCategoryId(firstCategoryId);
    ...
    public List<SharePostResponseDto> findPostsByCategoryId(Long categoryId) {
        ...
        List<SharePost> entities = sharePostRepository.findByCategoryId(categoryId);
        return entities.stream()
                .map(SharePostResponseDto::new)
                .collect(Collectors.toList());
    }
    ...
    public SharePostResponseDto findPostById(Long postId){
        SharePost entity = sharePostRepository.findById(postId)
    ...
    public SharePostResponseDto createPost(SharePostCreateRequestDto dto, Long userId){
        ...
        SharePost newPost = dto.toEntity(user, category);
        SharePost savedPost = sharePostRepository.save(newPost);
...
//    댓글 관련 서비스
    ...
    public List<ShareCommentResponseDto> findCommentsByPostId(Long postId){
        List<ShareComment> entities = shareCommentRepository.findBySharePostId(postId);
        return entities.stream()
    ...
    public ShareCommentResponseDto createComment(Long postId, ShareCommentCreateRequestDto dto, Long userId){
        SharePost sharePost = sharePostRepository.findById(postId)
                .orElseThrow(() -> new IllegalArgumentException("게시물을 찾을 수 없습니다: " + postId));

        User user = userRepository.findById(userId).orElseThrow(() -> new IllegalArgumentException("사용자를 찾을 수 없습니다"));

        ShareComment newComment = dto.toEntity(user, sharePost);
        ShareComment savedComment = shareCommentRepository.save(newComment);
    ...
    public ShareCommentResponseDto updateComment(Long commentId, ShareCommentUpdateRequestDto dto, Long userId){
        ...
        ShareComment updatedCommnet = shareCommentRepository.save(target);
